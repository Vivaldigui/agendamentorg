...
## Retomada atual — 12/09/2026

O painel de `reorganizacao-painel-recepcao` foi reintegrado ao Guia da CIN.
Use `main` atualizada para novos trabalhos e publicações. As seções abaixo são
históricas e não descrevem mais a branch ou o estado atual de produção.

A regressão veio do deploy de 11/09 às 20:11 BRT: a linha editorial ainda tinha
o painel antigo. Hosting publica `public/` inteira, incluindo os arquivos que
não foram editados na tarefa. Não publicar a partir das branches antigas.

Validação da integração: 280 testes de functions, 44 testes editoriais e
`npm --prefix editorial run verificar` aprovados. O HTML do painel, seu CSS e
seu JavaScript foram preservados da branch reorganizada. Os conflitos da home
mantêm os títulos H2 do Guia e os IDs/foco acessível do fluxo mais novo.
O bloco final de headers do painel reúne CSP restrito e `noindex, nofollow`.
A callable `cancelarAgendamentoAdmin` já está ativa em produção; esta recuperação
precisa apenas de deploy de Hosting, sem alteração de dados ou regras.

## Registro histórico — 27/08/2026
